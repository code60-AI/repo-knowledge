...
│                                                             │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  │
│  │rk-create │  │rk-search │  │rk-update │  │rk-delete │  │
...
2. **Scan**: Glob finds all code files; skips `node_modules`, `.git`, `dist`, `build`, `vendor`, `__pycache__`
...
两个 Agent 均由对应 Skill 按需派发，不持久运行。

Both agents are dispatched on-demand by their respective Skills and do not run persistently.
