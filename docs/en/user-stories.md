...
## US-2.1 -- Search with natural language
...
**Main Flow**
1. User types `/repo-knowledge:rk-search how to validate user identity`
...
8. Agent prepends the query string to `validate-token`'s aliases in `_index.md`
...
## US-4.2 -- Delete a knowledge base
...
## US-1.2 -- Per-function documentation
...
3. For each function, Agent generates a dedicated Markdown doc with: name, file path, what it does, code, parameters, return value, usage example, notes
...
## US-2.2 -- Progressive alias learning
...
## US-3.2 -- Auto-remove deleted files
...
## US-4.1 -- List all knowledge bases
